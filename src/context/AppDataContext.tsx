...
import { currentUserStorageKey, emptyState } from "@/lib/app-state";
import {
  bootstrapRemoteFromSeed,
  fetchRemoteState,
  insertContract,
  insertDevice,
  insertDocument,
...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  const [state, setState] = useState<AppState>(() =>
    isSupabaseConfigured ? emptyState : loadState() ?? seedState,
  );
    ...
    () => ({
      ...
      async updatePreRegistrationStatus(id, status) {
        if (!isSupabaseConfigured) {
          setState((s) => ({
            ...s,
            preregistrations: s.preregistrations.map((item) =>
      ...
      async addTransmission(payload) {
        if (!isSupabaseConfigured) {
          setState((s) => ({
            ...s,
            transmissions: [
              { id: uid("tr"), createdAt: new Date().toISOString(), ...payload },
              ...s.transmissions,
            ],
          }));
          return;
      ...
      async addDocument(payload) {
        if (!isSupabaseConfigured) {
          setState((s) => ({
            ...s,
            documents: [
              { id: uid("doc"), uploadedAt: new Date().toISOString(), ...payload },
              ...s.documents,
            ],
          }));
          return;
      ...
      async addDevice(payload) {
        if (!isSupabaseConfigured) {
          setState((s) => ({
            ...s,
            devices: [
              { id: uid("dev"), lastSeen: new Date().toISOString(), ...payload },
              ...s.devices,
            ],
          }));
          return;
      ...
      async addMessageThread(payload) {
        if (!isSupabaseConfigured) {
          setState((s) => ({
            ...s,
            messages: [
              { id: uid("msg"), updatedAt: new Date().toISOString(), ...payload },
              ...s.messages,
            ],
          }));
          return;
